export default class SceneCredits extends Phaser.Scene {
  ...
  create() {
    ...
    this.title = this.add.text(this.game.config.width * .5, 100, 'CREDITS', {
    ...
    this.textOne = this.add.text(this.game.config.width * .5, this.game.config.height * .30, 'Player ship By MillionthVector under CC');
    this.textOne.setInteractive();
    this.textOne.setOrigin(.5);

    this.textOne.on('pointerdown', () => {
      window.open('http://millionthvector.blogspot.de', '_blank')
    });

    this.textTwo = this.add.text(this.game.config.width * .5, this.game.config.height * .40, 'Lasers assets By Wenrexa');
    this.textTwo.setInteractive();
    this.textTwo.setOrigin(.5);
    
    this.textTwo.on('pointerdown', () => {
      window.open('https://wenrexa.itch.io/wenrexa-game-assets-1', '_blank');
    });

    this.textThree = this.add.text(this.game.config.width * .5, this.game.config.height * .5, 'Laser sounds By Triki Minut Interactive under CC');
    this.textThree.setInteractive();
    this.textThree.setOrigin(.5);
    
    this.textThree.on('pointerdown', () => {
      window.open('https://www.trikiminut.com', '_blank');
    });

    this.textFour = this.add.text(this.game.config.width *.5, this.game.config.height * .6, 'Enemies ships By Carlos Alface');
    this.textFour.setInteractive();
    this.textFour.setOrigin(.5);
